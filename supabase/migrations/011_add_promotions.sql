-- Migration 011: Add Buy X Get Y Free Promotions Support
-- Adds promotion configuration columns to products and historical promotion snapshot columns to sale_items

-- 1. Add promotion columns to products table
alter table if exists public.products
  add column if not exists promotion_enabled boolean default false,
  add column if not exists promotion_type text default 'BUY_X_GET_Y_FREE',
  add column if not exists promotion_buy_quantity numeric default 2,
  add column if not exists promotion_free_quantity numeric default 1,
  add column if not exists promotion_start_date text,
  add column if not exists promotion_end_date text,
  add column if not exists promotion_active boolean default true;

-- 2. Add promotion snapshot columns to sale_items table
alter table if exists public.sale_items
  add column if not exists paid_quantity numeric default 1,
  add column if not exists free_quantity numeric default 0,
  add column if not exists total_quantity numeric default 1,
  add column if not exists promotion_applied boolean default false,
  add column if not exists promotion_type text,
  add column if not exists promotion_buy_quantity numeric,
  add column if not exists promotion_free_quantity numeric;

-- 3. Update complete_sale to deduct stock by total_quantity and store promotion snapshot
