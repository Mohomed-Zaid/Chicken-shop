-- Migration 013: Retail and Wholesale Selling and Pricing System
-- 1. Extend products table with wholesale fields
alter table if exists public.products
  ...
  add column if not exists wholesale_min_quantity numeric default null;

-- Set retail_price to selling_price for existing products
update public.products
set retail_price = selling_price
where retail_price is null;

-- 1b. Extend chicken_cuts table with wholesale price per kg
alter table if exists public.chicken_cuts
  add column if not exists wholesale_price_per_kg numeric(12,2);

-- 2. Extend sales table with selling_mode
alter table if exists public.sales
  add column if not exists selling_mode text default 'RETAIL';

-- 3. Extend sale_items table with selling_mode and price_type
alter table if exists public.sale_items
  add column if not exists selling_mode text default 'RETAIL',
  add column if not exists price_type text default 'RETAIL';

-- 4. Update complete_sale RPC to record selling_mode and price_type snapshots
