...
export async function migrateLocalStorageToSupabase(): Promise<MigrationReport> {
  ...
  try {
  await upsertRows('chicken_cuts', cuts.map((item: ChickenItem) => ({ id: item.id, name: item.name, price_per_kg: item.pricePerKg, active: item.active }))); migrated.push(result('chicken_cuts', cuts))
